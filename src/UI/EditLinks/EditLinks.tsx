"use client";
import s from './EditLinks.module.scss';
...
const EditLinks = ({links, type}: { links: Link[], type: string}) => {
    ...
    const deleteLink = useMutation({
        mutationFn: (id: string) => {
            return api.delete(`/link/delete/${id}`, {
                headers: {
    ...
    const addOrUpdateLink = useMutation({
        mutationFn: (link: any) => {
            if(editing){
                link.id = editing.id;
                link.type = type;
    ...
    return (
        <div className={s.wrap}>
                    ...
                    <div className={s.editBullet} key={b.id}>
                        ...
                        <div className={s.bulletActions} >
                            ...
                            <Popconfirm
                                ...
                                onConfirm={() => deleteLink.mutate(b.id)}
                                okText="Да"
                                cancelText="Отмена"
            ...
            >
                <Form
                    layout={"vertical"}
                    onFinish={(values)=>addOrUpdateLink.mutate(values)}
                    form={form}
                >
                    <Form.Item name={"name"} label={"Название ссылки"}>
                        <Input/>
                    </Form.Item>
